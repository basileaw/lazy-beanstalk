    ...
    while guessing:
        console.print(
            Panel(
                ...
                title_align="left",
                padding=1,
                width=console.width - 2  # -2 to account for panel borders
                ),
            justify="center"
            )
        while guess.lower() != random_quote["author"].lower() and guesses > 0:
            ...
            if guess.lower() == random_quote["author"].lower():
                ...
                console.print(f"[bold red]Nope![/bold red]\n\nHere's a hint: The author was born on {birth_date} {birth_place}", justify="center", width=console.width)

            elif guesses == 2:
                console.print(f"[bold red]Wrong Again![/bold red]\n\nHere's another hint: The author's first name starts with: {random_quote['author'][0]}", justify="center", width=console.width)

            elif guesses == 1:
                # get the first letter of the last word in the author's name
                last_initial = random_quote['author'].split(" ")[-1][0]
                console.print(f"[bold red]YOUR'RE STILL WRONG![/bold red]\n\nHere's your last hint: The author's last name starts with: {last_initial}", justify="center", width=console.width)
